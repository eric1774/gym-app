...
plan: "01"
subsystem: calendar
tags: [calendar, navigation, db, types, ui]
dependency_graph:
  requires: []
  provides: [calendar-data-layer, calendar-screen, calendar-tab]
  affects: [TabNavigator, types/index.ts]
tech_stack:
  added: []
  patterns: [useFocusEffect-cancellation, stack-navigator-per-tab, local-date-filtering]
key_files:
  created:
    - src/db/calendar.ts
...
decisions:
  - UTC boundary with 1-day buffer + JS-side local date filtering for month queries
  - PR detection matches Phase 10 pattern: first-ever performance returns false
  - CalendarDayDetail registered in CalendarStackParamList now for Plan 02 to implement
metrics:
  duration: ~3min
