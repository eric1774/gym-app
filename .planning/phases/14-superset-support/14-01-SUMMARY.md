...
plan: 01
subsystem: programs
tags: [superset, exercises, ui, database, migration]
dependency_graph:
  requires: []
  provides: [superset-data-layer, superset-ui]
  affects: [DayDetailScreen, ExerciseTargetRow, programs.ts, migrations.ts]
tech_stack:
  added: []
  patterns: [sqlite-migration, runTransaction, ScrollView-for-small-lists, buildGroups-render-structure]
key_files:
  created: []
  modified:
    - src/types/index.ts
...
decisions:
  - "ScrollView chosen over FlatList in DayDetailScreen — program days have 4-8 exercises (never hundreds), ScrollView avoids nested FlatList complexity for superset group containers"
  - "Date.now() used as superset group ID — unique enough for local-only SQLite app; no UUID dependency needed"
  - "buildGroups() pure function transforms flat ProgramDayExercise[] into single/superset group render structure"
  - "supersetGroupId added as required field (not optional) on ProgramDayExercise — null means no superset"
metrics:
  duration: "~3 minutes"
