...
describe('runMigrations', () => {
  it('runs all 9 migrations on a fresh database (version 0)', async () => {
    ...
    // 9 migrations = 9 INSERT version calls
    for (let i = 0; i < 9; i++) {
    ...
    expect(insertVersionCalls.length).toBe(9);
  });
  ...
  it('skips already-applied migrations when at version 3', async () => {
    ...
    // migrations 4, 5, 6, 7, 8, 9 remaining
    for (let i = 4; i <= 9; i++) {
    ...
    // Only 6 migrations ran (4,5,6,7,8,9)
    expect(transaction).toHaveBeenCalledTimes(6);
    // 6 version records inserted
    ...
    expect(versions).toEqual([4, 5, 6, 7, 8, 9]);
  });
  ...
  it('bootstraps a pre-migration database (exercises table exists, no schema_version)', async () => {
    ...
    // Remaining migrations: 3,4,5,6,7,8,9
    for (let i = 3; i <= 9; i++) {
      mockExecuteSql.mockResolvedValueOnce(mockResultSet([], i));
    }
  ...
  it('runs migration DDL inside a transaction', async () => {
    ...
    // Only migration 9 remains
    mockExecuteSql.mockResolvedValueOnce(mockResultSet([], 9));

    await runMigrations(db);

    expect(transaction).toHaveBeenCalledTimes(1);
    // Migration 9 repairs program_week values
